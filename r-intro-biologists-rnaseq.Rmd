...
---
Need to install R >= 3.6 for ttBulk, tidyHeatmap. Also need to install tidyverse, devtools, edgeR (maybe plotly).
...
Hierarchical clustering (maybe skip and leave for reading, and have heatmap of diff express genes instead)

```{r eval=FALSE}
library(gplots)

# can get 500 most variable genes with
counts.norm.variable = counts.norm %>% filter_variable()

# need to convert to logCPM?

# select just the columns needed
high_var <- counts.norm.variable %>%
  select(sample, gene_symbol,`count scaled`)

# convert to wide format
high_var <- high_var %>% pivot_wider(names_from=sample, values_from=`count scaled`)

# convert to matrix
high_var <- high_var %>%
  as_matrix(rownames=gene_symbol) 

#check columns are in order in sampleinfo, as if so use group info
colnames(high_var)==sampleinfo$X1

# set colours
mycol <- colorpanel(1000,"blue","white","red")

# create heatmap
heatmap.2(high_var, col= mycol, labCol=sampleinfo$condition, trace="none", scale="row", dendrogram="column")
```
...
Maybe include plotly

```{r eval=FALSE}
# make interactive
library(plotly)
counts.plot <- counts.de %>% 
  filter(condition %in% c("bpreg", "blact")) %>%
  select(logFC,PValue,gene_symbol,is_de)
p <- ggplot(counts.plot, aes(x=logFC, y=-log10(PValue), colour=is_de, text=gene_symbol)) +
  geom_point()
ggplotly(p)
```
