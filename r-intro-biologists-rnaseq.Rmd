...
tidyBulk can automatically filter out lowly expressed genes. It uses the edgeR filterByExpr function described [here](https://f1000research.com/articles/5-1408). This will keep genes with ~10 counts in a minimum number of samples, the number of the samples in the smallest group. In this dataset the smallest group size is 2 samples. tidyBulk performs this filtering in the functions we will use `scale_abundance()` and `test_differential_abundance()` and we can take a look at it with `filter_abundant()`.

```{r}
# Filtering out lowly abundant genes 
counts.filt <- counts %>% filter_abundant(factor_of_interest = condition)
...
In the tidyBulk package the function scale_abundance() generates scaled counts, it will also perform the filtering.
...
counts.de
```
Now we have columns with our logFC and FDR P values. There is also an is_de column that indicates if the gene has FDR < 0.05.
...
We can count how many differentially expressed genes there are using the is_de column.

```{r}
counts.de %>% filter(is_de == TRUE) %>% 
  n_distinct()
```
