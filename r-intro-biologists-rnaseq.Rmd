...
counts.norm %>% 
  filter(lowly_abundant == FALSE) %>%
    ggplot(aes(x=log2(count_scaled +1), group=sample, color=condition)) +
    geom_density()
```
...
counts.norm %>% 
  filter(lowly_abundant == FALSE) %>%
    ggplot(aes(x=sample, y=log2(count_scaled + 1), fill=condition)) +
    geom_boxplot() +
  geom_hline(aes(yintercept = median(log2(count_scaled + 1)), colour = 'red'))
```
...
counts.de %>%
	...
	ggplot(aes(x = condition, y = log2(count_scaled + 1), colour = condition)) +
	geom_jitter() +
